...
def plot_fit(levels, levels_, xc0, ABRtime, epochs_means, epochs_sems, pst_range, fit_XC0m,
    ...
    for level in levels_:
        ...
        if epochs_means is not None:
            ...
            if norm_waveforms:
                ys = 1 / (y.max(axis=0) - y.min(axis=0))
            else:
                ys = 1
            # plotting just windowed data
            ax[0].plot(ABRtime[ii], y[ii] * ys * level_diff_mode + level, 'k', linewidth=1)

            ax[0].fill_between(ABRtime[ii],
                        np.squeeze(epochs_means[ii, i, 0] - epochs_sems[ii, i]) * 1e6 * ys * level_diff_mode + level,
                        np.squeeze(epochs_means[ii, i, 0] + epochs_sems[ii, i]) * 1e6 * ys * level_diff_mode + level,
                        color='lightgrey', alpha=.5)

            ax[0].plot(ABRtime[ii], epochs_means[ii, i, 1] * 1e6 * ys * level_diff_mode + level, color=colors[0],
                       linewidth=1)
            ax[0].plot(ABRtime[ii], epochs_means[ii, i, 2] * 1e6 * ys * level_diff_mode + level, color=colors[1],
                       linewidth=1)
            if pst_range is not None:
                ax[0].set_xlim(np.array(pst_range) * 1000)
    if norm_waveforms:
        ax[0].set_ylim((levels_[0]-level_diff_mode*.9, levels_[-1]+level_diff_mode*.9))
    ...
    if fit_XC0m is not None:
        ...
        if fit_XC0m['sigmoid_fit'] is not None:
            if fit_XC0m['bestFitType'] == 'sigmoid':
                ...
                l = f"sigmoid fit,\nnot used"
                lw = 1
            ax[1].plot(levels, fit_XC0m['sigmoid_fit']['yfit'], color=colors[0], lw=lw, label=l)
            if fit_XC0m['bestFitType'] == 'power law':
                ...
                lw = 1
                ls = '-'
            ax[1].plot(levels, fit_XC0m['power_law_fit']['yfit'], color=colors[1], lw=lw, label=l, ls=ls)
        if fit_XC0m['threshold'] is not None:
            if fit_XC0m['threshold'] is np.inf:
                ...
                l = None
            ax[1].axvline(fit_XC0m['threshold'], color=colors[2], linestyle='--', linewidth=1, label=l)
            ...
            ax[0].plot((time_range[0]-np.diff(time_range)*.25, time_range[1]+np.diff(time_range)*.05),
                       fit_XC0m['threshold']*np.ones(2), color=colors[2], linestyle='--', linewidth=1)
        ...
        ax[1].legend(loc='lower left', bbox_to_anchor=(.2, 0), frameon=True, fontsize=10 * fs_scale, framealpha=1)
...
def run_fit(file_path, XCsubargs = None):
    ...
    fit_results, figure = estimate_threshold(abr_single_trial_data, **XCsubargs)
